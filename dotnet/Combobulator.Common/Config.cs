...
namespace Combobulator.Common
{
    public static class Config
    {
        ...
        public static string CacheExpiration = ConfigurationManager.AppSettings["Cache.ApiCacheDurationHours"];
        ...
        public static string EmailMeResultsTemplate = ConfigurationManager.AppSettings["Email.Template"];
        ...
        public static string PhoneExpression = @"^(((\+44\s?\d{4}|\(?0\d{4}\)?)\s?\d{3}\s?\d{3})|((\+44\s?\d{3}|\(?0\d{3}\)?)\s?\d{3}\s?\d{4})|((\+44\s?\d{2}|\(?0\d{2}\)?)\s?\d{4}\s?\d{4}))(\s?\#(\d{4}|\d{3}))?$";
